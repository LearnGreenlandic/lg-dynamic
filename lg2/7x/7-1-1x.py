...
S.patterns.append([
	...
	sfx(C() | Grep(r'^(aallar.*QQU|akuere|socialize|ataasinnguaq.*TUR\+QQU|eqqaama|eqqarsatigǝ|ikiortә.*GE|ilagǝ|ilinniartip|malip|naapip|oqarfigǝ|paarǝ|paasi|pizza.*TUR\+QQU|qimap|sammisare|soqutige|taa$|taku|tusar|tutsiuteqqip.*QQU).*Ind\+1Sg\+3SgO') | Inv(r'(LIR\+QQU|TAR|NIQAR|SSA|GIARTUR|INNAQ|PAK|(tusar|paasi|qimap).*TAQ)'), '\t@PRED'),
	['.\t.\t@CLB'],
	])
S.patterns.append([
	...
	sfx(C() | Grep(r'Sem/(inst|Geo).*Lok') | Inv(r'(IP|KKUT|LIR\+TAQ|SSAQ|PAK|INNAQ|meeqqerivik.*Poss|ilinniarnertuunngorniarfik.*Poss)'), '\t@ADVL>'),
	sfx(C() | Grep(r'^(akuere|socialize|eqqaama|eqqarsatigǝ|ikiortә.*GE|ilagǝ|ilinniartip|malip|naapip|oqarfigǝ|paarǝ|paasi|qimap|sammisare|soqutige|taa$|taku|tusar).*SSA(\+NNGIT)?\+V\+Ind\+1Sg\+3PlO') | Inv(r'(LIR\+QQU|TAR|NIQAR|GIARTUR|INNAQ|PAK|(tusar|paasi|qimap).*TAQ)'), '\t@PRED'),
	['.\t.\t@CLB'],
	])
S.patterns.append([
	sfx(C() | Grep(r'(ippassaq|taamani|ullumi$)') | Inv(r'Lok'), '\t@CL-ADVL>'),
