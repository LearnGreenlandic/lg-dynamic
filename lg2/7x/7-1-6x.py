...
S.patterns.append([
	...
	sfx(C() | Grep(r'^(aallar.*QQU|akuere|socialize|ataasinnguaq.*TUR\+QQU|avip|eqqaama|eqqarsatigǝ|ikiortә.*GE|ilagǝ|ilinniartip|malip|naapip|oqarfigǝ|paarǝ|paasi|pizza.*TUR\+QQU|qimap|sammisare|soqutige|taa$|taku|tusar|tutsiuteqqip.*QQU).*Ind\+3Pl\+3SgO') | Inv(r'(LIR\+QQU|TAR|NIQAR|SSA|GIARTUR|INNAQ|PAK|(tusar|paasi).*TAQ)'), '\t@PRED'),
	['.\t.\t@CLB'],
	])
S.patterns.append([
	...
	sfx(C() | Grep(r'Sem/(inst|Geo).*Lok') | Inv(r'(IP|KKUT|LIR\+TAQ|SSAQ|PAK|INNAQ|meeqqerivik.*Poss|ilinniarnertuunngorniarfik.*Poss)'), '\t@ADVL>'),
	sfx(C() | Grep(r'^(akuere|socialize|avip|eqqaama|eqqarsatigǝ|ikiortә.*GE|ilagǝ|ilinniartip|malip|naapip|oqarfigǝ|paarǝ|paasi|qimap|sammisare|soqutige|taa$|taku|tusar).*SSA(\+NNGIT)?\+V\+Ind\+3Pl\+3PlO') | Inv(r'(LIR\+QQU|TAR|NIQAR|GIARTUR|INNAQ|PAK|(tusar|paasi).*TAQ)'), '\t@PRED'),
	['.\t.\t@CLB'],
	])
S.patterns.append([
	sfx(C() | Grep(r'(ippassaq|taamani|ullumi$)') | Inv(r'Lok'), '\t@CL-ADVL>'),
	sfx(C() | Grep(r'Sem/(H|socialize\+TAQ|think\+TAQ|teach\+TAQ).*N\+Abs\+Sg\+3PlPoss') | Inv(r'(NIQ|LIR\+TAQ|INNAQ|SSAQ|alla|eqqumiitsuliortoq|ilinniarnertooq|tusagassiortoq|sakkutooq|kalaaleq|qallunaaq|oqaasilerisoq|nukarleq|kingulleq|inersimasoq|inuusuttoq|ilinniartoq|angajoqqaaq)'), '\t@OBJ>'),
	sfx(C() | Grep(r'Sem/(inst|Geo).*Lok') | Inv(r'(IP|KKUT|LIR\+TAQ|SSAQ|PAK|INNAQ|meeqqerivik.*Poss|ilinniarnertuunngorniarfik.*Poss)'), '\t@ADVL>'),
	sfx(C() | Grep(r'^(aallar.*QQU|akuere|socialize|ataasinnguaq.*TUR\+QQU|avip|eqqaama|eqqarsatigǝ|ikiortә.*GE|ilagǝ|ilinniartip|malip|naapip|oqarfigǝ|paarǝ|paasi|pizza.*TUR\+QQU|qimap|sammisare|soqutige|taa$|taku|tusar|tutsiuteqqip.*QQU).*Ind\+3Pl\+3SgO') | Inv(r'(LIR\+QQU|TAR|NIQAR|SSA|GIARTUR|INNAQ|PAK|(tusar|paasi).*TAQ)'), '\t@PRED'),
	['.\t.\t@CLB'],
	])
