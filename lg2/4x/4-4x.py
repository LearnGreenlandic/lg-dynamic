...
S.patterns.append([
	...
	sfx(C() | Grep(r'^(angaju|ani|aqqalu|ataata|nuka|qatanngut|ui)\+Sem/Hfam\+GE\+Sem/have\+V\+Ind\+1Sg\+3SgO'), '\t@PRED'),
	['.\t.\t@CLB'],
...
def qa(sentence):
	...
	for w in sentence:
		...
		if w[0] == 'P1':
			...
			w[0] = sentence[1].split('\t')[0].replace('+V+Ind+1Sg+3SgO', '+V+Cont+3SgO')
			...
			w[0] = sentence[2].split('\t')[0].replace('+V+Ind+3Sg+3SgO', '+V+Cont+3SgO')
			if w[0] not in S.corpus_kv:
				return
			w[1] = S.corpus_kv[w[0]]
